...
public class RecipeService {
    ...
    log.info("gemini.image.enabled={} (property), effectiveApiKeyPresent={}", geminiImageEnabled, effectiveApiKey != null && !effectiveApiKey.isBlank());
        ...
        try {
            // 2. Execute the HTTP POST request (using WebClient) with exponential backoff retry
            // Increased max attempts for 503 Service Unavailable (Gemini overload)
            ...
            for (int attempt = 1; attempt <= maxAttempts; attempt++) {
                try {
                    ...
                    if (responseBody != null) {
                        log.info("Successfully received response from Gemini API on attempt {}", attempt);
                        break;
                    }
                    ...
                    try {
                        ...
                        log.debug("Failed to read response body from Gemini error: {}", ex.getMessage());
                    }
                    try {
                        String payloadStr = objectMapper.writeValueAsString(payload);
                        String payloadSnippet = payloadStr.length() > 1000 ? payloadStr.substring(0, 1000) + "..." : payloadStr;
                        String respSnippet = respBody == null ? "" : (respBody.length() > 200 ? respBody.substring(0, 200) + "..." : respBody);
                        log.error("Gemini API returned HTTP {} on attempt {}: {} -- responseSnippet={} -- payloadSnippet={}", statusCode, attempt, wcre.getMessage(), respSnippet, payloadSnippet);
                    } catch (Exception logEx) {
                        // Fall back to basic log if serialization failed
                        log.error("Gemini API returned HTTP {} on attempt {}: {} -- failed to serialize payload/response for logging ({} )", statusCode, attempt, wcre.getMessage(), logEx.getMessage());
                    }
                    ...
                    if (wcre.getStatusCode() == HttpStatus.FORBIDDEN) {
                        if (devFallback) {
                            ...
                            log.error("Received 403 Forbidden from Gemini API; devFallback disabled — rejecting request.");
                            throw new RuntimeException("Gemini API authentication failed (403). Please check API key configuration.");
                        }
                    }
                    
                    // For 503 Service Unavailable, use longer exponential backoff
                    if (wcre.getStatusCode() == HttpStatus.SERVICE_UNAVAILABLE) {
                        if (attempt >= maxAttempts) {
                            log.error("Gemini API still returning 503 after {} attempts - service may be overloaded", maxAttempts);
                            throw new RuntimeException("Gemini API is currently unavailable (503). Please try again in a few minutes.");
                        } else {
                            // Exponential backoff: 1s, 2s, 4s, 8s
                            long backoffMs = (long) (1000 * Math.pow(2, attempt - 1));
                            log.warn("Attempt {} failed with 503 Service Unavailable — retrying after {}ms", attempt, backoffMs);
                            try { Thread.sleep(backoffMs); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                            continue;
                        }
                    }
                    
                    // For other 5xx/429 errors, use standard retry
                    if (attempt >= maxAttempts) {
                        log.error("Gemini API returned HTTP {} on final attempt", statusCode);
                        throw new RuntimeException(String.format("Gemini API error (%d) after %d attempts", statusCode, maxAttempts));
                    } else {
                        long backoffMs = 500L * attempt;
                        log.warn("Attempt {} failed with HTTP {} — retrying after {}ms", attempt, statusCode, backoffMs);
                        try { Thread.sleep(backoffMs); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); }
                        continue;
                    }
                    ...
                    if (attempt >= maxAttempts) {
                        ...
                        throw new RuntimeException("Failed to connect to Gemini API after " + maxAttempts + " attempts", we);
                    } else {
                        long backoffMs = 500L * attempt;
            ...
            if (responseBody == null) {
                ...
                throw new RuntimeException("Failed to get response from Gemini API after " + maxAttempts + " attempts");
            }
